...
- 結果(イテレーション3・段階1、テスト不具合#1修正後): **58 passed**(`utils/__tests__/items.test.ts`4件、`InvoiceDetailPage`・`QuoteDetailPage`に数量が文字列の回帰各1件を追加。両画面の既存モックも実API形式の`'1.00'`へ変更)。直前(段階1時点): **52 passed**(既存37件+新規15件〈`ProjectKanbanPage`4件、`ProjectDetailPage`6件、`InvoiceDetailPage`・`QuoteDetailPage`・`ExpenseFormPage`への案件欄の回帰5件〉、失敗0件)。以下はイテレーション2時点の記録: **37 passed**(失敗0件。イテレーション1の30件+イテレーション2で追加した7件)
